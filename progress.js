(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // Reset button (footer): clears progress + registration on this device
    var resetBtn = document.getElementById('reset-progress-btn');
    if (resetBtn) {
      resetBtn.addEventListener('click', function () {
        var ok = window.confirm('Скинути прогрес і реєстрацію на цьому пристрої? Дію не можна відмінити.');
        if (!ok) return;
        try {
    ...
    // Test CTA gate: only unlocks once all 7 OP modules are completed
    // TODO: replace with the real Google Form URL once it's ready, and its
    // entry IDs for name/email so the test form opens pre-filled.
    var TEST_FORM_URL = 'https://docs.google.com/forms/d/e/1FAIpQLScS1EWzO5bCyKbO8eOQb137iyZ4edsOmbQnB6SWN6Xh04hkeA/viewform';
    var TEST_FORM_NAME_ENTRY = null;   // e.g. 'entry.111111'
    var TEST_FORM_EMAIL_ENTRY = null;  // e.g. 'entry.222222'

    var testBtn = document.getElementById('test-cta-btn');
    var testStatusText = document.getElementById('test-status-text');
    if (testBtn) {
      var doneCount = completedCount();
      var totalCount = MODULES.length;
      if (doneCount >= totalCount) {
        var url = TEST_FORM_URL;
        if (url !== '#' && TEST_FORM_NAME_ENTRY && TEST_FORM_EMAIL_ENTRY) {
          var emp = getEmployee();
          if (emp) {
            var sep = url.indexOf('?') === -1 ? '?' : '&';
            url += sep + 'usp=pp_url' +
              '&' + TEST_FORM_NAME_ENTRY + '=' + encodeURIComponent(emp.name || '') +
              '&' + TEST_FORM_EMAIL_ENTRY + '=' + encodeURIComponent(emp.contact || '');
          }
        }
        testBtn.href = url;
        ...
        testBtn.addEventListener('click', function (e) {
          e.preventDefault();
          window.alert('Спочатку потрібно пройти всі модулі з охорони праці (' + doneCount + ' з ' + totalCount + ' поки що пройдено).');
        });
        if (testStatusText) {
          var left = totalCount - doneCount;
          testStatusText.textContent = 'Залишилось пройти ще ' + left + ' з ' + totalCount + ' модулів, щоб розблокувати тест.';
        }
      }
    }
  });
